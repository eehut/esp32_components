...
// randombytes implementation using ESP-IDF's random number generator
void randombytes(u8 *x, u64 n)
{
    for (u64 i = 0; i < n; i += 4) {
        u32 r = esp_random();
        u64 remaining = n - i;
...
// Additional functions for detached signatures and key derivation

/**
 * @brief Derive public key from 32-byte private key
 * @param pk Output public key (32 bytes)
 * @param sk Input private key (32 bytes)
 * @return 0 on success, -1 on failure
 */
int crypto_sign_publickey_from_secret(u8 *pk, const u8 *sk)
{
  u8 d[64];
  gf p[4];
...
/**
 * @brief Create 64-byte secret key from 32-byte private key
 * @param sk64 Output secret key (64 bytes: 32 bytes private + 32 bytes public)
 * @param sk32 Input private key (32 bytes)
 * @return 0 on success, -1 on failure
 */
int crypto_sign_secretkey_from_private(u8 *sk64, const u8 *sk32)
{
  u8 d[64];
  gf p[4];
  int i;
  ...
  // Copy private key
  FOR(i, 32) sk64[i] = sk32[i];
  
  // Derive public key
  crypto_hash(d, sk32, 32);
  d[0] &= 248;
  d[31] &= 127;
  d[31] |= 64;
...
/**
 * @brief Ed25519 detached signature (signature only, without message)
 * @param sig Output signature (64 bytes)
 * @param siglen Output signature length pointer (set to 64)
 * @param m Message to sign
 * @param mlen Message length
 * @param sk Secret key (64 bytes: 32 bytes private + 32 bytes public)
 * @return 0 on success, -1 on failure
 */
int crypto_sign_detached(u8 *sig, u64 *siglen, const u8 *m, u64 mlen, const u8 *sk)
{
  u8 *sm = NULL;
  u64 smlen;
  int i;
  ...
  // Limit message size to reasonable value (e.g., 2KB for JWT)
  if (mlen > 2048) {
    return -1; // Message too large
  }
  
  // Allocate buffer on heap to avoid stack overflow
  // crypto_sign output: message + 64-byte signature
  sm = (u8 *)malloc(mlen + 64);
  if (sm == NULL) {
    return -1; // Allocation failed
  }
  
  // Use crypto_sign to get message + signature
